...
static int
setup_iso_data_path(uint16_t conn_handle)
{
    struct ble_hci_le_setup_iso_data_path_cp cp = { 0 };
    struct ble_hci_le_setup_iso_data_path_rp rp;
    uint16_t opcode;
    int rc;

    cp.conn_handle = le16toh(conn_handle);
    cp.data_path_dir = BLE_ISO_DATA_DIR_RX;

    opcode = BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SETUP_ISO_DATA_PATH);

    rc = ble_hs_hci_cmd_tx(opcode, &cp, sizeof(cp), &rp, sizeof(rp));
    printf("%s: rc=%d\n", __func__, rc);
...
gap_listener_func(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_HCI_LE_SUBEV_BIG_SYNC_ESTABLISHED:
        ...
        for (uint8_t bis = 0; bis < ev_est->num_bis; bis++) {
            setup_iso_data_path(le16toh(ev_est->conn_handle[bis]));
        }
...
{
    uint8_t buf[sizeof(struct ble_hci_le_big_create_sync_cp) + 2];
    struct ble_hci_le_big_create_sync_cp *cp;
    uint16_t opcode;
    int rc;

    cp = (struct ble_hci_le_big_create_sync_cp *)buf;
    cp->big_handle = 1;
    ...
    rc = ble_hs_hci_cmd_tx(opcode, buf, sizeof(buf), NULL, 0);
    printf("%s: rc=%d\n", __func__, rc);
