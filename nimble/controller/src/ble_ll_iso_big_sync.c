...
#define TP_PIN_EVENT        ARDUINO_PIN_D0
#define TP_PIN_TEST0        ARDUINO_PIN_D1
#define TP_PIN_ISR          ARDUINO_PIN_D2
#define TP_PIN_TEST1        ARDUINO_PIN_D3
#define TP_PIN_TEST2        ARDUINO_PIN_D4
#define TP_PIN_WFR_EXP      ARDUINO_PIN_D5
#define TP_PIN_TEST3        ARDUINO_PIN_D6
#else
#define TP_PIN_EVENT        0
#define TP_PIN_TEST0        0
#define TP_PIN_ISR          0
#define TP_PIN_TEST1        0
#define TP_PIN_TEST2        0
#define TP_PIN_WFR_EXP      0
#define TP_PIN_TEST3        0
#endif
...
static void
iso_conn_init(struct ble_ll_iso_conn *conn)
{
    struct ble_ll_isoal_mux_init_param mux_init_param;
    struct ble_ll_iso_big *big;
    struct ble_ll_iso_bis *bis;

    bis = CONTAINER_OF(conn, struct ble_ll_iso_bis, conn);
    big = bis->big;

    conn = &bis->conn;
    memset(conn, 0, sizeof(*conn));
    conn->rx = &bis->rx;
    conn->handle = BLE_LL_CONN_HANDLE(BLE_LL_CONN_HANDLE_TYPE_BIS_SYNC, bis->num);

    mux_init_param.iso_interval_us = big->iso_interval * 1250;
    mux_init_param.sdu_interval_us = big->sdu_interval;
    mux_init_param.max_sdu = big->max_sdu;
    mux_init_param.max_pdu = big->max_pdu;
    mux_init_param.bn = big->bn;
    mux_init_param.pte = 0;
    mux_init_param.framed = big->framed;
    mux_init_param.framing_mode = false;
    ble_ll_isoal_mux_init(&conn->mux, &mux_init_param);

    ble_ll_iso_conn_add(&bis->conn);
}

static void
iso_conn_free(struct ble_ll_iso_conn *conn)
{
    ble_ll_iso_conn_rem(conn);
...
static uint16_t
ble_ll_iso_big_sync_ww_calc(struct ble_ll_iso_big *big, uint32_t offset_us)
{
    /* Window widening to include drift due to sleep clock accuracy and jitter */
    return offset_us * big->sca / 1000000 + BLE_LL_JITTER_USECS;
}
...
static void
big_sync_sched_set(struct ble_ll_iso_big *big, uint32_t start_time, uint8_t remainder,
                   uint32_t offset_us, uint32_t big_ww_us)
{
    big->sch.start_time = start_time;
    big->sch.remainder = remainder;

    ble_ll_tmr_add(&big->sch.start_time, &big->sch.remainder, offset_us - big_ww_us);
    ...
    big->wfr_us = big_ww_us + big->tx_win_us;
}
             ...
             uint8_t sync_rem_us, const uint8_t *data, uint8_t len, void *arg)
{
    ...
    STAILQ_INIT(&big->bis_q);
    for (uint8_t bis_n = __builtin_ctz(u32); bis_n < bis_cnt; bis_n = __builtin_ctz(u32)) {
    ...
    anchor_offset_us = 0;
    while (1) {
        offset_us = big_offset_us + anchor_offset_us;
        ww_us = ble_ll_iso_big_sync_ww_calc(big, offset_us);

        big_sync_sched_set(big, big->anchor_base_ticks, big->anchor_base_rem_us, offset_us, ww_us);
        ...
        big->anchor_offset++;
        anchor_offset_us = big->anchor_offset * big->iso_interval * 1250;
        if (anchor_offset_us > BIG_SYNC_TIMEOUT_US(big->sync_timeout)) {
            /* Cannot synchronize on time */
            status = BLE_ERR_UNSPECIFIED;
            goto fail;
        }
    };
...
ble_ll_iso_big_sync_event_done(struct ble_ll_iso_big *big)
{
    ...
    do {
        ...
        big->anchor_offset++;
        anchor_offset_us = big->anchor_offset * big->iso_interval * 1250;
        ww_us = ble_ll_iso_big_sync_ww_calc(big, anchor_offset_us);

        big_sync_sched_set(big, big->event_start, big->event_start_us, anchor_offset_us, ww_us);
...
ble_ll_iso_big_sync_event_done_ev(struct ble_npl_event *ev)
{
    ...
    STAILQ_FOREACH(bis, &big->bis_q, bis_q_next) {
        ble_ll_iso_conn_event_done(&bis->conn);
    }
...
ble_ll_iso_big_sync_bis_subevent_rx(struct ble_ll_iso_big *big)
{
    ...
    to_rx = (big->tx.subevents_rem > 1) || big->cstf;

    /* Schedule next transition:
     * - RX - if there is another BIS Subevent or BIG Control
     * - NONE - if there is no BIG Control nor BIS Subevent
     */
...
ble_ll_iso_big_sync_ctrl_subevent_rx(struct ble_ll_iso_big *big)
{
    ...
    /* Schedule next transition:
     * - NONE - as BIG Control is the last event in BIG
     */
    ble_phy_transition_set(BLE_PHY_TRANSITION_NONE,
                           big->interleaved ? big->bis_spacing : big->sub_interval);
...
static int
ble_ll_iso_big_sync_rx_start(struct ble_ll_iso_big *big, uint32_t start_time, uint32_t remainder)
{
    int rc;
...
ble_ll_iso_big_sync_event_sched_cb(struct ble_ll_sched_item *sch)
{
    ...
    STAILQ_FOREACH(bis, &big->bis_q, bis_q_next) {
        ble_ll_iso_conn_event_start(&bis->conn,
                                    ble_ll_tmr_t2u(big->event_start) + big->event_start_us);
    ...
    big->tx.bis->subevent_num = 1;

    rc = ble_ll_iso_big_sync_rx_start(big, sch->start_time + g_ble_ll_sched_offset_ticks, sch->remainder);
    if (rc != 0) {
        TP_0(EVENT);
        goto done;
    }
...
ble_ll_iso_big_sync_rx_isr_start(uint8_t pdu_type, struct ble_mbuf_hdr *rxhdr)
{
    ...
    if (big->tx.subevents_rem == big->num_bis * big->nse) {
        big->anchor_offset = 0;
...
ble_ll_iso_big_sync_data_pdu_in(uint16_t conn_handle, uint8_t idx, struct os_mbuf *rxpdu)
{
    ...
    return ble_ll_iso_data_pdu_in(conn, idx, rxpdu);
}
...
ble_ll_iso_big_sync_rx_restart(struct ble_ll_iso_big *big)
{
    ...
    subevent_num = -1;
    STAILQ_FOREACH(bis, &big->bis_q, bis_q_next) {
        subevent_num += bis->subevent_num;
    }
    BLE_LL_ASSERT(subevent_num >= 0);

    offset_us += subevent_num * (big->interleaved ? big->bis_spacing : big->sub_interval);

    start_time = big->event_start;
    remainder = big->event_start_us;
    ble_ll_tmr_add(&start_time, &remainder, offset_us);

    return ble_ll_iso_big_sync_rx_start(big, start_time, remainder);
}
...
ble_ll_iso_big_sync_wfr_timer_exp(void)
{
    ...
    /* PDU not received. Mark the subevent as done, prepare for next RX if needed. */
    rc = ble_ll_iso_big_sync_subevent_done(big);
    if (rc != 0) {
        goto event_done;
    }
...
event_done:
    start_time = big->event_start;
    remainder = big->event_start_us;
    ble_ll_tmr_add(&start_time, &remainder, BIG_SYNC_TIMEOUT_US(big->sync_timeout));
    if (LL_TMR_LEQ(start_time, ble_ll_tmr_get())) {
...
ble_ll_iso_big_sync_hci_create(const uint8_t *cmdbuf, uint8_t len)
{
    ...
    if (rc == -EALREADY) {
        ble_ll_sync_biginfo_cb_set(syncsm, NULL, NULL);
        return BLE_ERR_CMD_DISALLOWED;
    }
    ...
    memcpy(sync_params->broadcast_code, cmd->broadcast_code, sizeof(sync_params->broadcast_code));
    sync_params->big = big;
...
int
ble_ll_iso_big_sync_hci_terminate(const uint8_t *cmdbuf, uint8_t len, uint8_t *rspbuf, uint8_t *rsplen)
{
...
ble_ll_iso_big_sync_init(void)
{
    ...
    rc = stats_init_and_reg(STATS_HDR(ble_ll_iso_big_sync_stats),
                            STATS_SIZE_INIT_PARMS(ble_ll_iso_big_sync_stats, STATS_SIZE_32),
                            STATS_NAME_INIT_PARMS(ble_ll_iso_big_sync_stats),
                            "ble_ll_iso_big_sync");
    BLE_LL_ASSERT(rc == 0);

    rc = os_mempool_init(&mp_big, BIG_POOL_SIZE, sizeof(struct ble_ll_iso_big), mb_big, "sbig");
    BLE_LL_ASSERT(rc == 0);
    rc = os_mempool_init(&mp_bis, BIS_POOL_SIZE, sizeof(struct ble_ll_iso_bis), mb_bis, "sbis");
    BLE_LL_ASSERT(rc == 0);
