...
static bool
ble_ll_sync_biginfo_cb_check(struct ble_ll_sync_sm *sm) {
#if MYNEWT_VAL(BLE_LL_ISO_BROADCAST_RECEIVER)
    return sm->biginfo_cb != NULL;
#endif
    return false;
}
...
ble_ll_sync_biginfo_handle(struct ble_ll_sync_sm *sm, struct ble_mbuf_hdr *hdr,
                           const uint8_t *biginfo, uint8_t biginfo_len, bool reports_enabled)
{
    uint8_t phy;

    /* The BIGInfo length is 33 octets for an unencrypted BIG and 57 octets for an encrypted BIG. */
    if (biginfo_len != BLE_LL_SYNC_BIGINFO_LEN && biginfo_len != BLE_LL_SYNC_BIGINFO_LEN_ENC) {
        return;
    }
...
#if MYNEWT_VAL(BLE_LL_ISO_BROADCAST_RECEIVER)
    if (sm->biginfo_cb) {
        sm->biginfo_cb(sm, sm->sca, hdr->beg_cputime, hdr->rem_usecs,
                       biginfo, biginfo_len, sm->biginfo_cb_arg);
    }
#endif
}
#endif
...
#if MYNEWT_VAL(BLE_LL_PERIODIC_ADV_SYNC_BIGINFO_REPORTS)
    if (biginfo && (reports_enabled || ble_ll_sync_biginfo_cb_check(sm))) {
        ble_ll_sync_biginfo_handle(sm, hdr, biginfo, biginfo_len, reports_enabled);
    }
#endif
