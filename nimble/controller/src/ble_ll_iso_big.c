...
static void
iso_conn_init(struct ble_ll_iso_conn *conn, struct big_params *bp, uint8_t idx, uint8_t pte)
{
    ...
    conn->tx = &bis->tx;
    conn->handle = BLE_LL_CONN_HANDLE(BLE_LL_CONN_HANDLE_TYPE_BIS, idx);

    mux_init_param.iso_interval_us = bp->iso_interval * 1250;
    mux_init_param.sdu_interval_us = bp->sdu_interval;
    mux_init_param.max_sdu = bp->max_sdu;
    mux_init_param.max_pdu = bp->max_pdu;
    mux_init_param.bn = bp->bn;
    mux_init_param.pte = pte;
    mux_init_param.framed = bp->framing != BLE_HCI_ISO_FRAMING_UNFRAMED;
    mux_init_param.framing_mode = bp->framing == BLE_HCI_ISO_FRAMING_FRAMED_UNSEGMENTED;
                      ...
                      struct big_params *bp)
{
    ...
    for (idx = 0; (big->num_bis < num_bis) && (idx < BIS_POOL_SIZE); idx++) {
        ...
        iso_conn_init(&bis->conn, bp, idx, pte);
    }
